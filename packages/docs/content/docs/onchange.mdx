...
- `displayText`: Shows mention labels (what users see, e.g., "@Alice")
- `rawText`: Shows mention values (actual data, e.g., "user_123")
...
```tsx
type MentionData = {
  displayText: string; // The text as displayed in the input (shows mention labels)
  rawText: string; // The text with mention values (shows actual data)
  mentions: Array<{
...
```tsx
const handleChange = (newValue: MentionData) => {
  // displayText shows what the user sees (e.g., "Hello @Alice and @Bob")
  console.log("Display text:", newValue.displayText);

  // rawText shows the actual values (e.g., "Hello user_123 and user_456")
  console.log("Raw text:", newValue.rawText);
