...
- `displayValue`: Shows mention labels (what users see, e.g., "@Alice")
...
```tsx
type MentionData = {
  displayValue: string; // The text as displayed in the input (shows mention labels)
...
```tsx
const handleChange = (newValue: MentionData) => {
  // displayValue shows what the user sees (e.g., "Hello @Alice and @Bob")
  console.log("Display text:", newValue.displayValue);
